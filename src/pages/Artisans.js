      ...
      <div style={styles.actionBar}>
        ...
        {statut === "suspendu" && (
          <>
            ...
            <button style={{ ...styles.btnAction, backgroundColor: "#E74C3C" }} onClick={() => { if (window.confirm("Supprimer cet artisan ?")) { fetch(API_URL + "/api/admin/artisans/" + artisan.id, { method: "DELETE" }).then(r => r.json()).then(d => { if (d.success) { alert("Supprime"); onClose(); window.location.reload(); } }); } }}>Supprimer</button>
          </>
        )}
      </div>
    </Modal>
